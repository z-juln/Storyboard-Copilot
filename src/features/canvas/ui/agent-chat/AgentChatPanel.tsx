...
import { UiButton, UiIconButton, UiPanel, UiSelect, UiTextArea } from '@/components/ui';
import {
...
export const AgentChatPanel = memo(({
  ...
  const { modelId, selectModel } = useAgentChatModel();
  ...
  const handleSend = useCallback(async () => {
    ...
    const result = await invokeAgentChatReply(messages, content, modelId);
    setIsSending(false);
  ...
  }, [activeConversationId, commitMessages, isSending, messages, modelId]);
  ...
  return (
    ...
    >
      <UiPanel className="pointer-events-auto flex max-h-[min(36rem,calc(100vh-8rem))] flex-col overflow-hidden rounded-xl shadow-xl">
        <div className="border-b border-border-dark px-3 py-2">
              ...
              <div className="w-24 shrink-0">
                <UiSelect
                  ...
                  onChange={(event) => {
                    ...
                    if (isAgentChatModelId(next)) {
                      selectModel(next);
                    }
                  }}
                >
                  {AGENT_CHAT_MODEL_OPTIONS.map((option) => (
          ...
          <p className="mt-1 text-[11px] leading-relaxed text-text-muted">
            内置 DeepSeek · 联网 · Enter 发送 · Shift+Enter 换行
          </p>
        </div>
