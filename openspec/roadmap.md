...
- [ ] 参数：帧数/间隔、智能模式开关
- [x] 大图走项目 `.cache` 预览（contentHash 键，节点不存 preview 字段）
...
- 图片局部修改：对选定区域做内容调整（inpainting / 局部重绘等，形式待定）
- 视频局部修改：对片段或局部区域做替换、修复或内容微调
- 参考视频驱动：如根据参考视频换角色、动作/表演迁移等（边界与输入输出待定）

**说明**：产品形态、节点类型与供应商能力仍在探索，落地前再拆 `changes/` 验收。
...
| 2026-06-20 | 新增「局部编辑与视频再创作」方向项（待细化） |
