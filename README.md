...
- 桌面容器：Tauri 2
- 后端：Rust 本地 HTTP API（`:1421`）+ Tauri invoke（窗口 / 部分图片与 AI）
- 项目数据：Project Bundle（`project.json` + `assets/`）；API Key 存 SQLite（`projects.db`）
...
src/
  ...
  features/project/         # 项目 Bundle 编解码与路径
  stores/                   # 全局状态与自动持久化策略
  commands/                 # 前端命令层（HTTP / Tauri invoke 桥接）
  infrastructure/           # rustApiClient（:1421 HTTP）
  i18n/                     # 国际化入口与语言包
src-tauri/src/
  http/                     # 本地 HTTP API（:1421）
  project/                  # Project Bundle 读写（file_store）
  ai/                       # Adapter 与 API Key（SQLite）
  commands/                 # Tauri invoke 命令（图片 / AI / 系统等）
  lib.rs                    # Tauri 入口与 HTTP 服务拉起
...
- 项目持久化：HTTP `rustApiClient` → `{app_data}/projects/<id>/project.json` + `assets/`
- 视口保存：独立 HTTP `PUT /api/v1/projects/:id/viewport`，不触发整项目 upsert
